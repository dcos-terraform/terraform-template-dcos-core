...
data "template_file" "config" {
  ...
  vars {
    ...
    dcos_agent_list                              = "${length(var.dcos_agent_list) > 0 ? "- ${join("\n- ",var.dcos_agent_list)}" : ""}"
    ...
    dcos_master_list                             = "${length(var.dcos_master_list) > 0 ? "- ${join("\n- ",var.dcos_master_list)}" : ""}"
    ...
    dcos_public_agent_list                       = "${length(var.dcos_public_agent_list) > 0 ? "- ${join("\n- ",var.dcos_public_agent_list)}" : ""}"
    dcos_previous_version                        = "${var.dcos_previous_version}"
    dcos_previous_version_master_index           = "${var.dcos_previous_version_master_index}"
    dcos_resolvers                               = "${length(var.dcos_resolvers) > 0 ? "- ${join("\n- ",var.dcos_resolvers)}" : ""}"
